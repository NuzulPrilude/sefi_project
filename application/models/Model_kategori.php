...
class Model_kategori extends CI_Model {
	// Tabel
	private $table = 'pj_kategori_barang pkb';
	...
	private $columnSearch 	= [
		'tmm.nama_mata_pelajaran', 
	];
	// Urukan
	private $orderBy 		= ['pkb.id_kategori_barang' => 'DESC'];
	// Select field
	...
	public function _setWhere()
	{
		...
		if($cari != ''){
			$this->db->like('pkb.kategori',$cari);
		}
	}
	// 
	...
	// 
	private function _statusUser($status_user_nama = null, $status_user_id = 1) 
	{
		if ($status_user_id == 1) {
	...
	private function _button($data) 
	{
		 $button = '<div class="btn-group pull-left">';
            ...
            $button .= '<li><a  href="' . site_url('admin/master/kategori/editkategori/' . md5($data->id_kategori_barang)) . '">Edit</a></li>';

            $button .= '<li><a onclick="return confirm(' . "'Apakah Anda Yakin?'" . ')" href="' . site_url('admin/master/kategori/hapuskategori/' . md5($data->id_kategori_barang)) . '">Hapus</a></li>';
	...
	public function getDataTables()
	{
		...
		foreach ($query as $field) {
		    $row    = array();
		    $row[]  = $no++;
	   	 	$row[]	= $field->kategori ? $field->kategori : '-';
